...
Moment Lab 內部自用工具：上載客戶相、填資料，一鍵產生 A4 尺寸嘅印刷用
包裝卡成品 PDF。**照片全程喺你部機／瀏覽器本機處理，唔會上傳去任何伺服器**——
...
- 版面係由原 Canva 樣板匯出 PDF 逐一量度復刻（A4，595.5 × 842.25 pt），
  已經同原版樣板逐像素疊圖核對過，刀模外框／背景卡內框尺寸準確
- 匯出 A4 尺寸成品 PDF（內部以 300dpi 製作，可以直接送印）
- 預覽畫面度會用粉紅色標出正面卡刀模外框、背景卡內框嘅實際 mm 尺寸，
  方便核對印刷／裁切規格；呢個標註淨係喺預覽度顯示，唔會出現喺匯出嘅 PDF
...
- 右上角 `CHANMANS’ WORK` 標誌可以隨時開啟或關閉，設定可連同其他資料暫存
...
- 「暫存目前資料」會將輸入欄位＋配色存喺瀏覽器 localStorage，下次打開
  自動帶返出嚟（相片本身唔會暫存）
- 「下載成品 PDF」用唔到（例如喺有安全限制嘅網頁框架入面睇）嘅話，
  改用「在新分頁開啟成品 PDF」，喺新分頁另存為
...
- `vendor/jspdf.umd.min.js` — [jsPDF](https://github.com/parallax/jsPDF)，MIT License
